...
$sqlicons = "SELECT * FROM icons WHERE status = 'ACTIVE' ORDER BY parent_id ASC, sortorder ASC, name ASC";
$queryicons = $dbicons->query($sqlicons);
while($dataicons = $queryicons->fetchArray(SQLITE3_ASSOC)) {$iconDropdown[] = $dataicons;};
if (!empty($iconDropdown)){$iconDropdown = parseTree($iconDropdown, $whichlist="active");}
//
//Icons admin section
//
	...
	echo '<div class="dropdown" style="position:relative">
		...
		echo "\n";
			...
			if ( empty($node['children']) ) {
				echo '<li><a class="iconItemSelection '.$node['value']['prefix'].' '.$node['value']['code'].' '.$node['value']['modifier'].'" type="button" data-prefix="'.$node['value']['prefix'].'" data-code="'.$node['value']['code'].'" data-modifier="'.$node['value']['modifier'].'" data-id="'.$node['value']['id'].'">&nbsp;'.$node['value']['name'].'</a></li>';
			}
