...
(function( factory ) {
	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		_mouseStop: function(event) {
			...
			this._pid_current = $(this.domPosition.parent).parent().attr("sort_id");
		...
		_intersectsWithSides: function(item) {
			// this only seems to fire when hovering over an object, vertical only
...
console.log("here");
			if (this.floating && horizontalDirection) {
				 //I don't see any time this is true
				return (
					(horizontalDirection === "right" && isOverRightHalf) ||
					(horizontalDirection === "left" && !isOverRightHalf)
				);
			} else {
				//hovering over items, will make item under mouse toggle up and down.
		...
		_clear: function() {
			...
			//relocate event
			if (!(this._pid_current === this._uiHash().item.parent().parent().attr("sort_id") &&
		...
		serialize: function(options) {
			...
			$(items).each(function() {
				var res = ($(o.item || this).attr(o.attribute || "sort_id") || "")
						.match(o.expression || (/(.+)[-=_](.+)/)),
					pid = ($(o.item || this).parent(o.listType)
						.parent(o.items)
						.attr(o.attribute || "sort_id") || "")
						.match(o.expression || (/(.+)[-=_](.+)/));
		...
		toHierarchy: function(options) {
			...
			function _recursiveItems(item) {
				var id = ($(item).attr(o.attribute || "sort_id") || "").match(o.expression || (/(.+)[-=_](.+)/)),
					currentItem;
				if (id) {
					currentItem = {
						"sort_id": id[2]
		...
		toArray: function(options) {
			...
			function _recursiveArray(item, depth, _left) {
				...
				id = ($(item).attr(o.attribute || "sort_id")).match(o.expression || (/(.+)[-=_](.+)/));
